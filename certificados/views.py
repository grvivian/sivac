        ...
        if request.user.is_superuser:
            if request.method == 'POST':
                ...
                if form.is_valid():
                    ...
                    return HttpResponseRedirect('/certificados/importar/')
                else:
                    messages.error(request, "Preencha corretamente o formulário.")
            else:
                form = FormCSV()
            ...
            contexto = {'form': form}
            return render(request, 'certificados/importar.html', contexto)
        else:
            return HttpResponseRedirect('/certificados/')
    else:
        return HttpResponseRedirect("/certificados/login/")
...
def alterar_senha(request):
    ...
    contexto = {"form": form}
    return render(request, 'certificados/alterar_senha.html', contexto)
        ...
        if form.is_valid():
            ...
            if user is not None:
                if user.perfil.redefinir_senha: 
                    ...
                    return HttpResponseRedirect("/certificados/alterar_senha/")
                login(request, user)
                return HttpResponseRedirect("/certificados/")
            else:
